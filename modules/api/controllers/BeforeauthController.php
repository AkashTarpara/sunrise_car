...
class BeforeauthController extends Controller
{
  ...
  public function actionTradeproapplication()
  {
    $model = new Tradeproapplication();
    ...
    if (isset($data['Tradeproapplication']) && is_array($data['Tradeproapplication'])) {
      $data = $data['Tradeproapplication'];
    }
    ...
    unset($data['authorized_signature']);

    $model->load($data, '');
    $model->status = Tradeproapplication::STATUS_PENDING;

    $st3Certificate = UploadedFile::getInstanceByName('st3_resale_certificate');
    if (!$st3Certificate) {
      $st3Certificate = UploadedFile::getInstance($model, 'st3_resale_certificate');
    }

    $taxCertificate = UploadedFile::getInstanceByName('tax_exemption_certificate');
    if (!$taxCertificate) {
      $taxCertificate = UploadedFile::getInstance($model, 'tax_exemption_certificate');
    }

    $authorizedSignature = UploadedFile::getInstanceByName('authorized_signature');
    if (!$authorizedSignature) {
      $authorizedSignature = UploadedFile::getInstance($model, 'authorized_signature');
    }

    if ($st3Certificate) {
      $savedFile = $this->saveTradeProDocument($st3Certificate);
      if ($savedFile === false) {
        Yii::$app->MyFunctions->JsonPrint(array('status' => 0, 'message' => Yii::t('app', 'Invalid ST-3 resale certificate file.')));
      }
      $model->st3_resale_certificate = $savedFile;
    }

    if ($taxCertificate) {
      $savedFile = $this->saveTradeProDocument($taxCertificate);
      if ($savedFile === false) {
        Yii::$app->MyFunctions->JsonPrint(array('status' => 0, 'message' => Yii::t('app', 'Invalid tax exemption certificate file.')));
      }
      $model->tax_exemption_certificate = $savedFile;
    }

    if ($authorizedSignature) {
      $savedFile = $this->saveTradeProDocument($authorizedSignature);
      if ($savedFile === false) {
        Yii::$app->MyFunctions->JsonPrint(array('status' => 0, 'message' => Yii::t('app', 'Invalid authorized signature file.')));
      }
      $model->authorized_signature = $savedFile;
    }

    if ($model->validate() && $model->save(false)) {
      $emailSent = $this->sendTradeProApplicationEmail($model);
      Yii::info('Trade pro application email sent result for application #' . $model->id . ': ' . ($emailSent ? 'sent' : 'not sent'), __METHOD__);
  ...
  private function sendTradeProApplicationEmail($model)
  {
    ...
    Yii::warning('Trade pro application email preparing for application #' . $model->id, __METHOD__);
    Yii::warning('Trade pro application email to: ' . ($toEmail ?: '[empty]'), __METHOD__);
    Yii::warning('Trade pro application email from: ' . ($fromEmail ?: '[empty]') . ' / ' . ($fromName ?: '[empty]'), __METHOD__);

    if (empty($toEmail)) {
      Yii::warning('application_email env value is empty; trade pro application email not sent.', __METHOD__);
      return false;
    }

    if (empty($fromEmail)) {
      Yii::warning('Sender email is empty; trade pro application email not sent.', __METHOD__);
      return false;
    }
    ...
    try {
      ...
      $message = Yii::$app->mailer
        ->compose(['html' => 'tradeproapplication'], ['model' => $model])
        ->setTo($toEmail)
        ->setFrom([$fromEmail => $fromName])
        ->setReplyTo($model->email ?: $fromEmail)
        ->setSubject('New Trade Pro Application - ' . $model->business_name);

      if (!empty($model->st3_resale_certificate) && file_exists(Yii::getAlias("@app") . "/" . $model->st3_resale_certificate)) {
        Yii::warning('Attaching ST-3 resale certificate: ' . $model->st3_resale_certificate, __METHOD__);
        $message->attach(Yii::getAlias("@app") . "/" . $model->st3_resale_certificate);
      } else if (!empty($model->st3_resale_certificate)) {
        Yii::warning('ST-3 resale certificate file not found: ' . $model->st3_resale_certificate, __METHOD__);
      }

      if (!empty($model->tax_exemption_certificate) && file_exists(Yii::getAlias("@app") . "/" . $model->tax_exemption_certificate)) {
        Yii::warning('Attaching tax exemption certificate: ' . $model->tax_exemption_certificate, __METHOD__);
        $message->attach(Yii::getAlias("@app") . "/" . $model->tax_exemption_certificate);
      } else if (!empty($model->tax_exemption_certificate)) {
        Yii::warning('Tax exemption certificate file not found: ' . $model->tax_exemption_certificate, __METHOD__);
      }

      if (!empty($model->authorized_signature) && file_exists(Yii::getAlias("@app") . "/" . $model->authorized_signature)) {
        Yii::warning('Attaching authorized signature: ' . $model->authorized_signature, __METHOD__);
        $message->attach(Yii::getAlias("@app") . "/" . $model->authorized_signature);
      } else if (!empty($model->authorized_signature)) {
        Yii::warning('Authorized signature file not found: ' . $model->authorized_signature, __METHOD__);
      }

      $sent = $message->send();
      Yii::warning('Trade pro application mailer send() returned: ' . ($sent ? 'true' : 'false'), __METHOD__);
      return $sent;
    } catch (\Exception $e) {
      Yii::error('Trade pro application email failed: ' . $e->getMessage(), __METHOD__);
      Yii::error($e->getTraceAsString(), __METHOD__);
      return false;
    }
  }

  public function actionTradeproapplicationsubmit()
  {
    return $this->actionTradeproapplication();
  }

  private function saveTradeProDocument($file)
  {
    ...
    $path = 'uploads/files/trade_pro_application/';
    if (!is_dir($path)) {
      mkdir($path, 0777, true);
      chmod($path, 0777);
    }
